...
class TAIParamDeviceName {
    ...
    String testONHarnessPath = label.OFAHarnessPath;
    ArrayList<String> bdrFunctionName;

    public TAIParamDeviceName() {
    }

    public TAIParamDeviceName(String filePath, String driverFilePath) {
        ...
        try {
            ...
            for (int i = 0; i < paramFileName.size(); i++) {
                ...
                if (deviceNameMatch.find()) {
                    ...
                    while (!paramFileName.get(j).equals("</COMPONENT>") && j < paramFileName.size() - 1) {
                        ...
                        Pattern deviceCoordinatePattern = Pattern.compile("\\<coordinate>(.*)</coordinate\\>");
                        ...
                        Pattern devicehostNamePattern = Pattern.compile("\\<hostname\\>(.*)\\</hostname\\>");

...
#Tests the startup of Zookeeper1, RamCloud1, and ONOS1 to be certain that all started up successfully
    ...
    def CASE33(self,main):
        ...
        srcSw = main.params['SWITCH']['sw1']
        dstSw = main.params['SWITCH']['sw7']
        
        intentPort = main.params['INTENTREST']['intentPort']
        intentCount = main.params['INTENTS']['intentCount']
        intAddDur = main.params['intAddDur']
        intentURL = main.params['INTENTREST']['intentURL']

        intentHigh1 = "http://"+ONOS1_ip+":"+intentPort+intentURL
        intentHigh2 = "http://"+ONOS2_ip+":"+intentPort+intentURL
        intentHigh3 = "http://"+ONOS3_ip+":"+intentPort+intentURL

        add_sleep = main.params['SLEEP']['addIntent']
        rem_sleep = main.params['SLEEP']['remIntent']
        ...
        for i in range(1,101):
            time.sleep(float(add_sleep))
           
            #Continue to obtain json object in the loop 
            #and save the highest saturation value
            ...
            temp_list1.append(int(rate1))
            temp_list2.append(int(rate2))
            temp_list3.append(int(rate3))

            main.log.info(
                "Current instance rates : \n"+
                "ONOS1: "+str(rate1)+"\n"+
                "ONOS2: "+str(rate2)+"\n"+
                "ONOS3: "+str(rate3)+"\n") 

            #Progress counter and status for a long sleep
            sys.stdout.write("\rProgress: "+ str(i)+"%")
            sys.stdout.flush()

        aggr_rate = [max(temp_list1),
                    max(temp_list2),
                    max(temp_list3)]

        main.log.info("Max rate list: " + str(aggr_rate))
        ...
        for i in range(1, 101):
            time.sleep(float(rem_sleep))
            
            json_obj1 = main.ONOS1.get_json(intentHigh1)
            json_obj2 = main.ONOS2.get_json(intentHigh2)
            json_obj3 = main.ONOS3.get_json(intentHigh3)

            #If all json objects are empty
            if len(json_obj1) == 0:
                if len(json_obj2) == 0:
                    if len(json_obj3) == 0:
    ...
    def CASE43(self, main):
        ...
        intentHigh2 = "http://"+ONOS2_ip+":"+intentPort+intentURL
        intentHigh3 = "http://"+ONOS3_ip+":"+intentPort+intentURL
        intentHigh4 = "http://"+ONOS4_ip+":"+intentPort+intentURL
        ...
        for i in range(1, 8): 
            main.Mininet4.handle.sendline("sudo ovs-vsctl del-controller s"+str(i))
        ...
        main.ONOS1.handle.sendline("~/./loadgen_NB1.py -n 13 -u "+
                ONOS1_ip+":"+intentPort+" -i "+intentCountNode+" -a 1 -l 135 -p 3"+
                " -s "+srcSw+" -d "+dstSw)
        main.ONOS2.handle.sendline("~/./loadgen_NB2.py -n 14 -u "+
                ONOS2_ip+":"+intentPort+" -i "+intentCountNode+" -a 1 -l 135 -p 3"+
                " -s "+srcSw+" -d "+dstSw)
        main.ONOS3.handle.sendline("~/./loadgen_NB3.py -n 15 -u "+
                ONOS3_ip+":"+intentPort+" -i "+intentCountNode+" -a 1 -l 135 -p 3"+
                " -s "+srcSw+" -d "+dstSw)
        main.ONOS4.handle.sendline("~/./loadgen_NB4.py -n 16 -u "+
                ONOS4_ip+":"+intentPort+" -i "+intentCountNode+" -a 1 -l 135 -p 3"+
                " -s "+srcSw+" -d "+dstSw)
        ...
        for i in range(1, 101):
            ...
            json_obj2 = main.ONOS2.get_json(intentHigh2)
            json_obj3 = main.ONOS3.get_json(intentHigh3)
            json_obj4 = main.ONOS4.get_json(intentHigh4)

            #If all json objects are empty
            if len(json_obj1) == 0 and len(json_obj2) == 0:
                if len(json_obj3) == 0 and len(json_obj4) == 0:
                    main.log.info("Intents successfully removed")
                    break
    ...
    def CASE53(self, main):
        ...
        intentHigh3 = "http://"+ONOS3_ip+":"+intentPort+intentURL
        intentHigh4 = "http://"+ONOS4_ip+":"+intentPort+intentURL
        intentHigh5 = "http://"+ONOS5_ip+":"+intentPort+intentURL
        ...
        main.ONOS5.handle.sendline("~/./loadgen_NB5.py -n 21 -u "+
                ONOS5_ip+":"+intentPort+" -i "+intentCountNode+" -a 1 -l 135 -p 3"+
                " -s "+srcSw+" -d "+dstSw)
        ...
        for i in range(1, 101):
            ...
            json_obj2 = main.ONOS2.get_json(intentHigh2)
            json_obj3 = main.ONOS3.get_json(intentHigh3)
            json_obj4 = main.ONOS2.get_json(intentHigh4)
            json_obj5 = main.ONOS3.get_json(intentHigh5)
    ...
    def CASE63(self, main):
        ...
        intentHigh4 = "http://"+ONOS4_ip+":"+intentPort+intentURL
        intentHigh5 = "http://"+ONOS5_ip+":"+intentPort+intentURL
        intentHigh6 = "http://"+ONOS6_ip+":"+intentPort+intentURL
        ...
        main.ONOS6.handle.sendline("~/./loadgen_NB6.py -n 27 -u "+
                ONOS6_ip+":"+intentPort+" -i "+intentCountNode+" -a 1 -l 135 -p 3"+
                " -s "+srcSw+" -d "+dstSw)
        ...
        for i in range(1, 101):
            ...
            json_obj3 = main.ONOS3.get_json(intentHigh3)
            json_obj4 = main.ONOS4.get_json(intentHigh4)
            json_obj5 = main.ONOS5.get_json(intentHigh5)
            json_obj6 = main.ONOS6.get_json(intentHigh6)
        
            #If all json objects are empty
            if len(json_obj1) == 0 and len(json_obj2) == 0:
                if len(json_obj3) == 0 and len(json_obj4) == 0:
                    if len(json_obj5) == 0 and len(json_obj6) == 0:
                        main.log.info("Intents successfully removed")
                        break
    ...
    def CASE73(self, main):
        ...
        intentHigh5 = "http://"+ONOS5_ip+":"+intentPort+intentURL
        intentHigh6 = "http://"+ONOS6_ip+":"+intentPort+intentURL
        intentHigh7 = "http://"+ONOS7_ip+":"+intentPort+intentURL
        ...
        main.ONOS7.handle.sendline("~/./loadgen_NB7.py -n 35 -u "+
                ONOS7_ip+":"+intentPort+" -i "+intentCountNode+" -a 1 -l 135 -p 3"+
                " -s "+srcSw+" -d "+dstSw)
        ...
        for i in range(1, 101):
            ...
            json_obj5 = main.ONOS5.get_json(intentHigh5)
            json_obj6 = main.ONOS6.get_json(intentHigh6)
            json_obj7 = main.ONOS7.get_json(intentHigh7)
    ...
    def CASE105(self,main):
        ...
        data = main.TRUE
        utilities.assert_equals(expect=main.TRUE,actual=data,
                onpass="Posting to database succeeded",
                onfail="Posting to database failed")

...
class RamCloudCoorCliDriver(CLI):
    '''
RamCloudCliDriver is the basic driver which will handle the RamCloud server functions
    '''
    def __init__(self):
    ...
    def status(self):
        ...
        response = self.execute(cmd="~/ONOS/start-ramcloud-coordinator.sh status ",prompt="\d+\sinstance\sof\sramcloud\srunning(.*)",timeout=10)
    ...
    def isup(self):
        ...
        Tries 5 times to call start-ramcloud-coordinator.sh status
        ...
        for i in range(tries):
            ...
            self.handle.sendline("")
            self.handle.expect("\$") 
            self.handle.sendline("~/ONOS/start-ramcloud-coordinator.sh status")

...
class OVXCliDriver(CLI):
    ...
    def LinkConnect(self, **largs):
        main.log.info ("Checking connectLink API")
        args = utilities.parse_args(["NWID", "DPID", "PORTNO", "DEST_DPID", "DEST_PORT", "ROUTE_TYPE", "BACKUP_PATH"], **largs)
        print "NWID =%s, SRC_DPID =%s, SRC_PORT =%s, DEST_DPID= %s, DEST_PORT=%s " %(args['NWID'], args['DPID'], args['PORTNO'], args['DEST_DPID'], args['DEST_PORT'])
        print "ROUTE_TYPE =%s, BACKUP_PATH=%s" %(args['ROUTE_TYPE'], args['BACKUP_PATH'])
	nw_id = str(args['NWID']) if args["NWID"] != None else ""
	src_dpid = str(args['DPID']) if args['DPID'] != None else ""
	src_port = str(args['PORTNO']) if args['PORTNO'] != None else ""
        dest_dpid = str(args['DEST_DPID']) if args['DEST_DPID'] != None else ""
        dest_port = str(args['DEST_PORT']) if args['DEST_PORT'] != None else ""
        route_type = str(args['ROUTE_TYPE']) if args['ROUTE_TYPE'] != None else ""
        backup_path = str(args['BACKUP_PATH']) if args['BACKUP_PATH'] != None else ""

        command = './ovxctl.py -n connectLink' + " " + nw_id + " " + src_dpid + " " + src_port + " " + dest_dpid + " " + dest_port + " " + route_type + " " + backup_path
        print "command = ", command
        response = self.execute(cmd=command, prompt="utils\$", timeout=30)
        print "response = ", response
        print "***********************"
        if " has been created" in response:
                return main.TRUE
        else:
                return main.FALSE

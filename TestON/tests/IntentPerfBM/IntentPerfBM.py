#########
#This script is a copy of 'IntentPerf'
#Only slight modifications are made to accommodate for 
#baremetal machines, hence the suffix 'BM' in the testname
#########
...
#******************************************
    def CASE2(self, main):
        import time
        ctrl1_bm = main.params['CTRL']['ip1_bm']
        ctrl2_bm = main.params['CTRL']['ip2_bm']
        ctrl3_bm = main.params['CTRL']['ip3_bm']
        port1 = main.params['CTRL']['port1']
        port2 = main.params['CTRL']['port2']
        port3 = main.params['CTRL']['port3']

        main.log.report("Assign switches to controllers")
        assertion = main.TRUE
        for i in range(1,8):
            if i < 3:
                main.Mininet1.assign_sw_controller(sw=str(i),ip1=ctrl1_bm,port1=port1)
                time.sleep(5) 
            elif i < 6 and i > 2:
                main.Mininet1.assign_sw_controller(sw=str(i),ip1=ctrl2_bm,port1=port2)
                time.sleep(5)
            elif i < 8 and i > 5:
                main.Mininet1.assign_sw_controller(sw=str(i),ip1=ctrl3_bm,port1=port3)
                time.sleep(5)
        for j in range(1,8):
        ...
        utilities.assert_equals(expect=main.TRUE,actual=assertion,
                onpass="Switches assigned successfully",onfail="Switches NOT assigned")
...
#***************************************
    def CASE4(self,main):
        ...
        for i in range(0,int(numIter)): 
            ...
            main.ONOS2.tshark_grep("OFP 130", tshark_output, interface='eth4')
            time.sleep(10) 
...
#********************************
    def CASE6(self, main):
        ...
        for i in range(0, int(numIter)):
            ...
            main.ONOS2.tshark_grep("OFP 130", tshark_port_batch, interface='eth4')
            time.sleep(10)            

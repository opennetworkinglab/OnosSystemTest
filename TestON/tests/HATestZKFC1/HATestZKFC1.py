...
class HATestZKFC1:
    ...
    def CASE5(self,main):
        ...
        main.step("2 Zookeeper Server (including leader) Failure!")
        result = main.TRUE
        ...
        if re.search("leader",master1):
            ...
            main.ZK2.kill()
            time.sleep(10)
            if re.search("leader",main.ZK3.status()) or \
                    re.search("leader",main.ZK4.status()) or \
                    re.search("leader",main.ZK5.status()):
                result = main.TRUE
            ...
            if re.search("leader",master2):
                ...
                main.ZK3.kill()
                time.sleep(10)
                if re.search("leader",main.ZK1.status()) or \
                        re.search("leader",main.ZK4.status()) or \
                        re.search("leader",main.ZK5.status()):
                    result = main.TRUE
                    main.log.info("New Leader Elected")
                ...
                if re.search("leader",master3):
                    ...
                    time.sleep(10)
                    if re.search("leader",main.ZK1.status()) or \
                            re.search("leader",main.ZK2.status()) or \
                            re.search("leader",main.ZK5.status()):
                        result = main.TRUE
                        main.log.info("New Leader Elected")
                    else:
                    ...
                    if re.search("leader",master4):
                        ...
                        if re.search("leader",main.ZK1.status()) or \
                                re.search("leader",main.ZK2.status()) or \
                                re.search("leader",main.ZK3.status()):
                            result = main.TRUE
                            main.log.info("New Leader Elected")
                        else:
                            result = main.FALSE
                        ...
                        if re.search("leader",main.ZK3.status()) or \
                                re.search("leader",main.ZK4.status()) or \
                                re.search("leader",main.ZK2.status()):
                            result = main.TRUE
                            main.log.report("New Leader Elected")
                        else:
                            result = main.FALSE
        utilities.assert_equals(expect=main.TRUE,actual=result,
                onpass="New Leader was Elected!",
                onfail="No new leader was elected")
    ...
    def CASE7 (self,main):
        ...
        time.sleep(5)
        Link_Discovery = main.ONOS1.check_status_report(ONOS1_ip,active,str(int(links)-2))
        ...
        if result == main.TRUE:
            main.log.report("Link failure discovered correctly")
        utilities.assert_equals(expect=main.TRUE,actual=result,
    ...
    def CASE8 (self, main) :
        ...
        time.sleep(31)
        Del_Switch_Discovered = main.ONOS1.check_status_report(ONOS1_ip,str(int(active)-1),str(int(links)-4))
        ...
        main.Mininet1.assign_sw_controller(sw="28",count=5,
                ...
                ip5=ONOS5_ip,port5=ONOS5_port)
        time.sleep(31)
        Add_Switch_Discovered = main.ONOS1.check_status_report(ONOS1_ip,active,links)
